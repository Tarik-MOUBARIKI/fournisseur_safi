...
// get all sellers 
  ...
  const deleteCategory = (id)=>{
    axios.delete(`http://localhost:5050/superAdmin/deleteCategorie/${id}`)
    .then(function (response) {
        ...
        <div className="flex h-screen antialiased text-black bg-gray-400">
          <aside className="flex-shrink-0 p-4 w-72 h-full">
            <div className="flex flex-col h-full pt-12 pb-4 rounded-lg neumorphism-shadow bg-white">
              ...
              <span href="" target="_blank" className="mt-3 px-4 py-1 rounded-md text-xl font-semibold tracking-wider text-white-600  focus:outline-none">{SuperAdmin}</span>
              <span href="" target="_blank" className="mt-3 px-4 py-1 rounded-md text-xl font-semibold tracking-wider text-white-600  focus:outline-none">Super Admin</span>
            </div>
                ...
                <ul className="max-h-full p-2 space-y-1 overflow-y-auto divide-y divide-blue-300 neumorphism-shadow">
                  ...
                  <li>
                    <Link to='/fournisseur' className="flex items-center px-4 py-2 text-gray-600 transition-transform transform rounded-md hover:translate-x-1 focus:ring focus:outline-none">
                      ...
                      <span className="ml-2 font-medium">Space Fournisseurs</span>
                    </Link>
                  </li>
                  {/* Category link */}
                  <li>
                    <Link to='/category' className="flex items-center px-4 py-2 text-gray-600 transition-transform transform rounded-md hover:translate-x-1 focus:ring focus:outline-none">
                      ...
                      <span className="ml-2 font-medium">Space Category</span>
                    </Link>
                  </li>
                </ul>
              </nav>
...
<div className="overflow-x-auto" style={{width:1200}}>
  ...
  <div className=" flex items-center justify-center font-sans " style={{marginTop:150}}>
    ...
    <Link to='/addCategory' class="bg-blue-300 hover:bg-green-100 text-black font-bold py-2 px-4 rounded-100">New Category</Link>
      <div className="bg-white shadow-md rounded my-6">
        <table className="min-w-max w-full table-auto">
          <thead>
            <tr className="bg-gray-200 text-gray-600  text-sm leading-normal">
            <th className="py-3 px-6 text-center">Image Category</th>
              <th className="py-3 px-6 text-left">Name Category</th>
              <th className="py-3 px-6 text-center">Descreption</th>        
          ...
          <tbody className="text-gray-600 text-sm font-light">
            ...
            <tr className="border-b border-gray-200 hover:bg-gray-100">
              ...
              <td className="py-3 px-6 text-left whitespace-nowrap">
                  <img alt="" src=  {item.imageCategorie} style={{borderRadius:"10px", width:"150px",height:"100px"}}/>
              ...
              <textarea style={{height:'100px',width:'400px'}}>{item.description}</textarea>
{/*               
              </td> */}
              <td className="py-3 px-6 text-center">

                <div className="flex item-center justify-center">               
                  <Link onClick={()=>getIdCategory(item._id)}  className="w-4 mr-2 transform hover:text-yellow-500 hover:scale-110">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" />
                    </svg>
                  </Link>
                  <Link    onClick={()=>deleteCategory(item._id)}  className="w-4 mr-2 transform hover:text-red-500 hover:scale-110">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
