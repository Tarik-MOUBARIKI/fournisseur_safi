...
const  EditProductPromo = () => {
    ...
    const idProduct=localStorage.getItem('idProductPromo');

    // ---------------------get Product to update-----------------------------
useEffect(()=>{

    axios.get(`http://localhost:5050/fournisseur/getProductPromoById/${idProduct}`)
...
//---------Edit Product------------- 
    ...
    const Product = {titel,description,oldPrice,newPrice,productImg,quantity,category:categorySelected,fournisseur:idF,currentDate};

	axios.put(`http://localhost:5050/fournisseur/updateProductPromo/${idProduct}`,Product)
    .then(res => {
    if(res.error){
      return false
    }else{
      console.log(res.data);
      history.push('/listProductPromo')
    }
    ...
    <div className="relative sm:max-w-sm w-full">
      ...
      <div className="relative w-full rounded-3xl  px-6 py-4 bg-gray-100 shadow-md">
        ...
        <form method="#" action="#" className="mt-10" onSubmit={handleSubmit}>
          ...
          <div className="mt-7">
            <input type="text" placeholder="Old Price" className="mt-1 block w-full border-none bg-gray-100 h-11 rounded-xl shadow-lg hover:bg-blue-100 focus:bg-blue-100 focus:ring-0"  required 
            value={oldPrice}
            onChange={e => setOldPrice(e.target.value)}/>
          </div>
          <div className="mt-7">
            <input type="text" placeholder="New Price" className="mt-1 block w-full border-none bg-gray-100 h-11 rounded-xl shadow-lg hover:bg-blue-100 focus:bg-blue-100 focus:ring-0"  required 
            value={newPrice}
